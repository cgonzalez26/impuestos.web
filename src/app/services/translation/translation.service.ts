...
export class TranslationService {
  ...
  constructor(private translateService: TranslateService) { 
    this.translateService.get([
      ...
      'COMMON.MESSAGES.REQUEST_PROCESSED_SUCCESSFULLY']).subscribe(res => {
      ...
      this.requestProcessedSuccessfully = res['COMMON.MESSAGES.REQUEST_PROCESSED_SUCCESSFULLY'];
    });
  }

  public translate(key: string, value: string, callback?: any) {
    this.translateService.get(key, {value: value}).subscribe((res: string) => {
        if (callback) {
            callback(res);
        }
    });
